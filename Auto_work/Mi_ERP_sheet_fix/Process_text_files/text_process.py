...
import os
import re
from Auto_work.Mi_ERP_sheet_fix.Process_text_files import htm_file_warp
import openpyxl
import datetime
...
class ReadTranslatedTxt(object):
    ...
    def main(self):
        ...
        if check_if_the_same_day(get_time_stamp()):
            ...
            with open('../../Mi_ERP_sheet_fix/excr_node.py', 'w', encoding='utf-8') as t:
                t.write('excr_node = ' + str(excr_node))
            with open('../../Mi_ERP_sheet_fix/_time_stamp_for_excr_node.py', 'w', encoding='utf-8') as t:
                t.write('time_stamp = ' +
...
def get_time_stamp() -> datetime.datetime:
    if os.path.isfile('../../Mi_ERP_sheet_fix/_time_stamp_for_excr_node.py'):
...
def asking_for_excr_node_input() -> str:
    with open('../../Mi_ERP_sheet_fix/ExchangeRate_Node.txt', 'w', encoding='utf-8') as f:
        from excr_node import excr_node
        for key, value in excr_node.items():
            f.write(f'{key}: {value.replace("!![", "").replace("]!!", "")}\n')
    os.startfile('../../Mi_ERP_sheet_fix/ExchangeRate_Node.txt')
    is_finished = input('是否完成输入(Y/N):')
    if (is_finished == 'y') and (open('../../Mi_ERP_sheet_fix/ExchangeRate_Node.txt', 'r', encoding='utf-8').read() is not None):
        return open('../../Mi_ERP_sheet_fix/ExchangeRate_Node.txt', 'r', encoding='utf-8').read().strip()
    else:
        print('未完成输入，尝试重新输入...')
        file = open('../../Mi_ERP_sheet_fix/ExchangeRate_Node.txt', 'r', encoding='utf-8')
        file.close()
